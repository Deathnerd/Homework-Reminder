	...
	try {
		...
		switch ($action) {
			case "new":
				...
				break;
			case "delete":
				$statement = $db->prepare("DELETE FROM reminders WHERE class_name=:class_name AND assignment=:assignment;");
				break;
			case "done":
				$statement = $db->prepare("UPDATE reminders SET due=1 WHERE class_name=:class_name AND assignment=:assignment;");
				break;
			default:
				exit("Action not set");
		}
